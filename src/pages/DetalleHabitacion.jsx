...
function DetalleHabitacion() {
  ...
  async function registrarPago() 
  {
    ...
    const totalPagado = pagos.reduce((s, p) => s + parseFloat(p.monto), 0) + parseFloat(montoPago)
    const nuevoPago = totalPagado >= parseFloat(hospedaje.tarifa_pactada) ? 'pagado' : 'parcial'

    await supabase
      .from('hospedajes')
      .update({ estado_pago: nuevoPago })
      .eq('id', hospedaje.id)
  ...
  const saldo = hospedaje ? parseFloat(hospedaje.tarifa_pactada) + totalConsumos - totalPagado : 0
  ...
  return (
    <div className="p-4">
        ...
        <>
          ...
          <div className="bg-white rounded-xl border p-4 mb-3">
            ...
            <p className="text-xs text-gray-400 mt-1">
              Ingreso: {new Date(hospedaje.ingreso).toLocaleString('es-PE')}
            </p>
            <p className="text-xs text-gray-400">
              Checkout: {new Date(hospedaje.salida_estimada).toLocaleString('es-PE')}
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl border p-4 mb-3">
            ...
            <div className="flex justify-between text-sm py-1">
              <span>Hospedaje</span>
              <span>S/{hospedaje.tarifa_pactada}</span>
            </div>
            {totalConsumos > 0 && (
              <div className="flex justify-between text-sm py-1">
                <span>Consumos</span>
          ...
          {mostrarPago ? (
            ...
            <>
            <button
            onClick={() => navigate(`/consumos/${id}`)}
              className="w-full py-3 bg-orange-500 text-white rounded-xl font-semibold mb-3"
            >
              Consumos
            </button>
            <button
              onClick={() => setMostrarPago(true)}
              className="w-full py-3 bg-blue-600 text-white rounded-xl font-semibold mb-3"
            >
              Registrar pago
            </button>
            </>
      ...
      {(hab.estado === 'pendiente_limpieza' || hab.estado === 'en_limpieza') && (
        <div className="bg-white rounded-xl border p-4">
          <p className="text-gray-600 text-sm font-medium">Pendiente de limpieza</p>
          <p className="text-xs text-gray-400 mt-1">El módulo de limpieza se construye más adelante</p>
        </div>
      )}
