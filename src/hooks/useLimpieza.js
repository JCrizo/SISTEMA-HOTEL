...
export function useLimpieza() {
  ...
  const habilitarHabitacion = async (hab, horaFin) => {
    try {
      ...
      // FIX L1: también liberar si estaba en limpieza_simple o pendiente_limpieza (bypass directo)
      if (['en_limpieza', 'limpieza_simple', 'pendiente_limpieza'].includes(hab.estado)) {
